...
//CBState includes both states for the game and to__ states for movement
public enum CBState{
	drawpile,
	toHand,
	hand,
...
public class CardBartok : Card {
	...
	public void MoveTo(Vector3 ePos, Quaternion eRot){
		...
		//If timeStart is 0, then it's set to start mmediately,
		//otherwise, it starts at timeStart. This way, if timeStart is already set, it won't be overwritten.
		if (timeStart == 0) {
			timeStart = Time.time;
		}
		//timeDuration alway starts the same but can be altered later
	...
	void Update(){
		...
		case CBState.to:
			...
			if (u < 0){
				//If u<0, then we shouldn't move yet. Stay at the initial position
				...
				if (reportFinishTo != null){
					...
					//If there is nothing to callback, do nothing
				}
			} else {
